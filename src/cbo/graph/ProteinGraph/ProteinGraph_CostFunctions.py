...
def cost_R_fix_equal(intervention_value, **kwargs):
    fix_cost = 1.
    return fix_cost


def cost_M_fix_equal(intervention_value, **kwargs):
    fix_cost = 1.
    return fix_cost

def cost_Y_fix_equal(intervention_value, **kwargs):
    fix_cost = 1.
    return fix_cost
# ## Define a cost variable for each intervention
...
def define_costs(type_cost):
    ...
    #! test
    #name = ['S', 'A', 'C']
    #costs = OrderedDict([(name[i], cost_S_fix_equal) for i in range(3)])
    if type_cost==1:
        costs=OrderedDict([
            ('C',cost_C_fix_equal),
            ('A',cost_A_fix_equal),
