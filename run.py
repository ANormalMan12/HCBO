...
if __name__ == '__main__':
    ...
    if(args.run_performance):
        print("Begin to use baseline")
        if(not args.HDO_only):
            all_async_result_dict.update(HCBO_BaselineRunner(target_sem,ECCIS,cost,is_linear,x_y_history,pool).run_all(
                result_saver=result_saver,
                repeated_times=repeated_run_times,
                run_CBO=run_CBO,
                full_observational_samples=full_observational_samples,
                ECCIS_limit10=ECCIS_limit10
            ))
        for key,result in all_async_result_dict.items():    
            ...
            if not result.successful():
                ...
                print("Yes in ",key)
        if(not args.HCBO_only):
            report_info=get_hcbo_report(result_saver,'HCBO',target_sem.task=='max')
            best_y=report_info['HCBO']['best_Y']
            best_dim=report_info['HCBO']['best_dim']
            print(best_y)
            print(best_dim)
            if(run_CBO):
                init_data_num=3
            else:
                init_data_num=SYNT_INIT_INTERVENTION_DATA_NUM
            all_async_result_dict.update(optimize_cgo_with_all_hdo(
                target_sem=target_sem,
                init_data_num=init_data_num,
                optimization_cost=cost,
                intrinsic_dimension=best_dim,
                best_value=best_y,
                repeated_times=repeated_run_times,
                save_manager=result_saver,
                is_linear=is_linear,
                pool=pool
            ))
    pool.close()
